...
class LanguageModel(object):
    def __init__(self, num_labels, seq_length,num_layers,
        ...
        inputs = [tf.matmul(tf.squeeze(i, axis=[0]), w_i) + b_i
                  for i in tf.split(axis=0, num_or_size_splits=seq_length, value=self.inputs)]

        # set rnn init state to 0s
        initial_state = cell.zero_state(self.batch_size, tf.float32)

        # build rnn
        rnn_output, hidden_state = tf.contrib.rnn.rnn(cell, inputs,
        ...
        # compute logits
        self.logits = [tf.matmul(tf.squeeze(i, axis=[0]), w_o) + b_o
        ...
        if forward_only:
            ...
            self.losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits,
                                                                  labels=self.target,
                                                                  name="losses")
